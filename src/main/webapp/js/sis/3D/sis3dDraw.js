    ...
    Sis3DDraw.prototype = {
        ...
        startDraw: function(type= "line", isMeasure = false) {
            ...
            this.handler.setInputAction(function(evt) {
                ...
                if(!Cesium.defined(self.floatingPoint)) {
                    self.floatingPoint = self.drawPoint(newPosition, {
                        size: 10,
        ...
        getArea: function(entity, self) {
            ...
            if (positions.length > 0) {
                ...
                // var id = `draw${self.drawIdx}`;

                // var ray = self.map.scene.camera.getPickRay(Cesium.SceneTransforms.wgs84ToWindowCoordinates(self.map.scene, center));
                // var newPosition = self.map.scene.globe.pick(ray, self.map.scene);

                // var tooltip = `<div id="${id}" class="drawTooltip">${area}</div>`;
                // $(".cesium-widget").append(tooltip);
                //
                // self.map.scene.preRender.addEventListener(function() {
                //
                //     canvasPosition = self.map.scene.cartesianToCanvasCoordinates(
                //         newPosition,
                //         scratch
                //     );
                //
                ...
                //         $(`#${id}`).css({
                //             top: (canvasPosition.y - 14) + "px",
                //             left: (canvasPosition.x - ($(`#${id}`).outerWidth() / 2)) + "px"
                //         });
                //     }
                // });

                self.drawLabel(center, area);
            }
        },
